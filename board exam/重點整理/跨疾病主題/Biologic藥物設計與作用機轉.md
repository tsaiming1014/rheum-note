...
**原則二：Cytokine 濃度易劇烈波動時 → 打 receptor 對訊息阻斷更穩定**
...
理論上 anti-IL-6（中和游離 ligand）與 anti-IL-6R（阻斷 membrane-bound + soluble 兩種 receptor）都能同時擋掉 classic 和 trans-signaling，因為兩條路徑共用同一個上游 ligand。真正讓 anti-receptor 策略較被偏好的原因是藥物動力學上的**可飽和性（saturability）**：

- IL-6 是發炎急性期會劇烈波動、大量生成的 cytokine，anti-cytokine 抗體必須以莫耳數超過（molar excess）持續變動甚至暴增的游離 IL-6，才能維持完全中和；若上游生成量突然暴衝，固定劑量的抗體可能來不及蓋過，導致中和不完全。
- Receptor（不論 membrane-bound 或 soluble）的量相對穩定、有限，是較「可飽和」的標靶。只要藥物劑量足以飽和這個相對固定的 receptor pool，就能持續阻斷下游訊息傳遞，較不受上游 ligand 濃度暴衝影響。
...
**對照：Etanercept 效果不如 adalimumab/infliximab 的機轉**

Etanercept 是 soluble TNFR2-Fc fusion protein（decoy receptor），同時中和 TNF-α 和 lymphotoxin-α（LT-α，因為 LT-α 本來就是天然 TNFR2 的 ligand 之一，這點 adalimumab/infliximab 完全不擋）。儘管 etanercept 涵蓋的 ligand 種類其實更廣，臨床上在 uveitis、Crohn's disease/IBD 效果仍不如 adalimumab、infliximab（Crohn's 試驗甚至直接失敗，Sandborn et al. 2001），關鍵不在「擋了幾種分子」，而在跟 membrane-bound TNF 的結合方式與後續效應不同：

- **結合動力學（binding kinetics）**：Adalimumab/infliximab 是 bivalent IgG 抗體，對 TNF 親和力高、解離速率（off-rate）慢，能交聯多個 TNF trimer 形成穩定的大型免疫複合體；etanercept 本質上是模擬天然 receptor 去競爭 ligand，結合快速可逆、解離速率快很多。
- **Reverse signaling / 誘導凋亡能力**：membrane-bound TNF 本身也是一種「receptor」，被高親和力、穩定交聯的抗體結合時會把訊號往細胞內傳（outside-to-in reverse signaling），誘導表現 TNF 的活化巨噬細胞/T 細胞走向 apoptosis，這對清除腸道、眼睛內肉芽腫性發炎浸潤（granulomatous inflammation，Crohn's 與許多 uveitis 的病理核心）特別重要。Etanercept 因結合快速可逆、不易形成穩定交聯複合體，誘導這種 reverse signaling/凋亡的能力弱很多，等於暫時擋住 TNF 卻沒清除持續製造 TNF 的發炎細胞。
- **Fc-mediated 清除效率**：穩定的大型抗體-TNF 免疫複合體較容易活化補體（CDC）並被 Fc receptor 介導吞噬清除，etanercept 快速解離的交互作用較難達到同等效果。

（次要因素：etanercept 半衰期較短（~4 天 vs adalimumab ~2 週），需一週兩次注射，但多數學者認為這只是加分項，不是 Crohn's 治療失敗的主因——即使調高劑量頻率，etanercept 在 IBD 的試驗依然失敗。）
