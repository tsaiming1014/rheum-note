...
## 3. 緩解 CAR-T 副作用的設計策略（第14題相關）

CAR-T最主要的副作用是**CRS（cytokine release syndrome，細胞激素釋放症候群）**與**ICANS（immune effector cell-associated neurotoxicity syndrome，神經毒性）**，以下為降低風險的設計：

| 設計 | 機轉 | 目的 |
|---|---|---|
| **iCaspase9（誘導性Caspase9）安全開關** | 給予二聚化藥物（如AP1903）後誘導CAR-T細胞快速凋亡 | CRS/神經毒性發生時可迅速「關閉」治療，屬於**自殺基因（suicide gene）**策略 |
| **Syn-Notch / Bispecific（雙特異性）設計** | 需同時辨識兩種抗原才活化，或先辨識抗原A誘導表現CAR再辨識抗原B才殺傷 | 提高腫瘤專一性、降低on-target/off-tumor毒性 |
| **NKG2D CAR** | 以NKG2D天然受體取代scFv，辨識多種腫瘤壓力配體（如MICA/MICB、ULBP） | 拓寬辨識範圍，但專一性設計與傳統scFv CAR不同，非直接針對CRS |
| **TRUCK** | 額外分泌cytokine強化抗腫瘤能力 | 目的是增強效果而非降低副作用，**反而可能加重全身性發炎反應** |

> 因此第14題「可緩解CAR-T副作用之CAR設計」答案應選 **iCaspase9**（安全開關），其餘三者皆屬於「增強效果」而非「降低副作用」的設計方向。
